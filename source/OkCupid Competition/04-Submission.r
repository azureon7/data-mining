...
# per guardarle si pu?? andare nella cartella source
...
for(i in 1:20){
  fit.tot <- rfb.fit(X_train, y_train, theta=1, alpha=0.20, B=B, 
...
# SUBMISSION
write.table(file="./submission/OkCupid_Submission.txt", y_pred_finale, row.names = FALSE, col.names = FALSE)
